...
namespace YouSoftBathReception
{
    class GeneralClass
    {
        ...
        //启用手牌锁
        public static bool use_seat_lock(BathDBDataContext db)
        {
            var ops = db.Options.ToList();
            if (ops.Count == 0 || ops[0].启用手牌锁 == null || !Convert.ToBoolean(ops[0].启用手牌锁))
                return false;
            return true;
        }
    }
    ...
    class GetStringSpell
    {
        ...
        /// 
        #region ChineseCharList

        private static string[] ChineseCharList = 
        { 
            ...
            "D饾鬥梪脰酘痘閗窦鬦餖斣闘竇鬪鬬鬭嘟督醏毒涜读渎椟牍犊裻読蝳獨錖凟匵嬻瀆櫝殰牘犢瓄皾騳黩讀豄贕韣髑鑟韇韥黷讟厾独笃堵帾琽****睹覩賭篤芏妒杜肚妬度荰秺渡靯镀螙殬鍍蠧蠹偳",
            ...
            "M擟糜縻麊麋靡獼麛爢戂攠蘼醾醿鸍釄米羋芈侎弭洣敉粎脒眯渳葞蝆蔝銤孊灖糸汨宓泌觅峚祕宻诡秘淧覓覔幂谧塓幎覛嘧榓滵漞熐蔤蜜鼏冪樒幦濗藌謐櫁簚羃芇眠婂绵媔棉綿緜臱蝒嬵檰櫋",
            ...
            "Q偂掮揵軡媊鈐鉗墘榩箝銭潜橬錢黔鎆黚騝濳騚灊籖鰬浅肷淺嵰慊遣蜸潛谴缱繾譴鑓欠刋芡茜倩悓堑傔嵌棈椠皘蒨塹歉蔳儙槧篏輤篟壍嬱縴呛羌戕戗斨****玱猐琷跄嗴椌獇腔嗆溬蜣锖嶈戧槍",
        ...
        public static string GetChineseSpell(string strText)
        {
            ...
            foreach (char vChar in strText)
            {
                ...
                {
                    // 若字符Unicode编码在编码局限则 查汉字列表进行转换输出
                    foreach (string strList in ChineseCharList)
                    {
